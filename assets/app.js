(() => {
  ...
  function renderMap(){
    ...
    const paths=svg.append('g').selectAll('path').data(world.features||[]).enter().append('path').attr('class',f=>`world-country${featureCountry(f)?' is-visited':''}`).attr('d',path)
      ...
      .on('mouseenter',function(f){const c=featureCountry(f);tip.html(c?`<strong>${esc(c.name)}</strong><br>${c.visits} visits · ${(c.regions||[]).length} regions`:`${esc(f?.properties?.name||'')}`).classed('is-visible',true);})
  ...
  function renderPlacePanel(){const panel=$('#place-panel');if(!panel||!state.selected)return;const p=state.selected;let html='';
    if(p.type==='country'){
      const cityCount=(p.regions||[]).reduce((s,r)=>s+(r.cities||[]).length,0);html=`<div class="place-panel__crumb">Country / ${esc(p.continent)}</div><div class="place-panel__flag">${flag(p.iso2)}</div><h3>${esc(p.name)}</h3><p>${esc(p.essence||'')}</p><div class="place-panel__stats"><div><strong>${p.visits||0}</strong><span>visits</span></div><div><strong>${(p.regions||[]).length}</strong><span>regions</span></div><div><strong>${cityCount}</strong><span>cities</span></div><div><strong>${journeys.filter(j=>j.country===p.iso3).length}</strong><span>stories</span></div></div><span class="micro">GO DEEPER</span><div class="place-panel__list">${(p.regions||[]).map(r=>`<button data-select-region="${esc(p.iso3)}|${esc(r.name)}"><span>${esc(r.name)}</span><span>→</span></button>`).join('')}</div>`;
    } else if(p.type==='region') {const c=countryByIso.get(p.countryIso);html=`<div class="place-panel__crumb">${esc(c?.name||'')} / Region</div><h3>${esc(p.name)}</h3><p>${esc(c?.essence||'')}</p><div class="place-panel__stats"><div><strong>${(p.cities||[]).length}</strong><span>cities</span></div><div><strong>${(p.cities||[]).reduce((s,x)=>s+(x.visits||0),0)}</strong><span>city visits</span></div></div><span class="micro">CITIES</span><div class="place-panel__list">${(p.cities||[]).map(city=>`<button data-select-city="${esc(p.countryIso)}|${esc(p.name)}|${esc(city.name)}"><span>${esc(city.name)}</span><span>${city.visits||0}×</span></button>`).join('')}</div>`;
    } else {const c=countryByIso.get(p.countryIso);const js=journeys.filter(j=>j.country===p.countryIso&&j.city===p.name);html=`<div class="place-panel__crumb">${esc(c?.name||'')} / ${esc(p.regionName||'')} / City</div><h3>${esc(p.name)}</h3><p>${esc(p.feeling||'')}</p><div class="place-panel__stats"><div><strong>${p.visits||0}</strong><span>visits</span></div><div><strong>${js.length}</strong><span>stories</span></div></div>${js.length?`<span class="micro">JOURNEYS</span><div class="place-panel__list">${js.map(j=>`<button data-open-journey="${esc(j.id)}"><span>${esc(j.title)}</span><span>→</span></button>`).join('')}</div>`:''}`;}
